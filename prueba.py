...
# Definir las opciones del menú de papitas con precios
menupapitas = {
    "a": {"nombre": "Papitas Fritas", "precio": 1.25},
    "b": {"nombre": "Papitas con queso", "precio": 1.75},
    "c": {"nombre": "Papitas con salsa", "precio": 2.00}
}
...
def imprimir_carrito(carrito):
    if carrito:
        ...
        for item, cantidad in carrito.items():
            ...
            elif item in menupapitas:
                nombre = menupapitas[item]["nombre"]
                precio_unitario = menupapitas[item]["precio"]
            precio_total = precio_unitario * cantidad
            print(f"{cantidad} {nombre} - ${precio_total:.2f}")
        total = sum(
            (menuham[item]["precio"] if item in menuham else menuhotdog[item]["precio"] if item in menuhotdog else menupapitas[item]["precio"]) * cantidad
            for item, cantidad in carrito.items()
        )
...
while True:
    cliente = input("¿A cuál de nuestras secciones desea entrar? Hamburguesas (a), Hot-Dogs (b), Papitas (c), Ver Carrito (d), Imprimir vóucher : ").lower()

    if cliente in menu:
        if cliente == "a":
            print("Le entregamos nuestro menú de hamburguesas:")
            mostrar_menu_seccion(menuham)  # Mostrar el menú de hamburguesas
            opcion_hamburguesa = input("Por favor, elija una opción (a),(b),(c): ").lower()
            if opcion_hamburguesa in menuham:
                confirmacion = input("¿Cuántas desea agregar al carrito?: ").lower()
                try:
        ...
        # Proceso de selección de hot-dogs
            ...
            opcion_hotdog = input("Por favor, elija una opción (a),(b),(c),(d): ").lower()
            if opcion_hotdog in menuhotdog:
                confirmacion = input("¿Cuántos desea agregar al carrito?: ").lower()
                try:
                    cantidad = int(confirmacion)
                    if cantidad > 0:
        ...
        # Proceso de selección de papitas
        elif cliente == "c":
            print("Le entregamos nuestro menú de papitas:")
            mostrar_menu_seccion(menupapitas)
            opcion_papitas = input("Por favor, elija una opción (a),(b),(c): ").lower()
            if opcion_papitas in menupapitas:
                confirmacion = input("¿Cuántas desea agregar al carrito?: ").lower()
                try:
                    cantidad = int(confirmacion)
                    if cantidad > 0:
                        # Agregar las papitas al carrito
                        if opcion_papitas in carrito:
                            carrito[opcion_papitas] += cantidad
                        else:
                            carrito[opcion_papitas] = cantidad
                        print(f"{cantidad} papita(s) agregada(s) al carrito.")
                    else:
                        print("Por favor, ingrese una cantidad válida.")
                except ValueError:
                    print("Por favor, ingrese un número válido.")
        ...
        # Imprimir el carrito
            ...
            print("DEBUG: Opción Ver Carrito seleccionada")
